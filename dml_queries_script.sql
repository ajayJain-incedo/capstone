...
select pname, product_id, quantity, amount
from cart_item, product
where user_id = 1001 and cart_item.product_id = product.pid ;
...
insert into order_history (user_id,order_id,product_id,amount)
select user_id,order_id,product_id,amount
from cart_item 
...
select pname, order_id, product_id, amount
from order_history, product
where user_id = 1001 and order_history.product_id = product.pid ;
